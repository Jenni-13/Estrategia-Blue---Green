...
if [ "$CURRENT_COLOR" == "blue" ]; then
    ...
    # Fallback si .env está vacío o corrupto
    echo "ERROR: CURRENT_DEPLOYMENT no encontrado o no válido. Desplegando en BLUE por defecto."
    NEW_COLOR="blue"
fi

echo "Desplegando la nueva versión en el entorno INACTIVO: $NEW_COLOR"

# 2. Despliegue del nuevo entorno usando el script específico
...
# 3. Health Check (Nos aseguramos de que el contenedor se esté ejecutando)
